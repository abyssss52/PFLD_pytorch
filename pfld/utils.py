...
def calculate_pitch_yaw_roll(landmarks_2D, cam_w=256, cam_h=256,
    ...
    c_x = cam_w / 2
    c_y = cam_h / 2
    f_x = c_x / np.tan((30 * 2 * np.pi) / (2 * 180))# np.tan(60 / 2 * np.pi / 180)
    f_y = f_x
    # camera_matrix = np.float32([[f_x, 0.0, c_x], [0.0, f_y, c_y],
    #                             [0.0, 0.0, 1.0]])
    camera_matrix = np.float32([[6.4008292339587180e+002, 0.0, 5.6000000000000000e+001], [0.0, 6.4008292339587180e+002, 5.6000000000000000e+001],
                                [0.0, 0.0, 1.0]])
